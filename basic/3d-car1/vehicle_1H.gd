...
#   esto pa ver el nombre del scrip
#	var script_path = get_script().resource_path
#	print("Nombre del script: ", script_path)
...
#	Game Over por caida con HUD:
	if global_transform.origin.y <  -20 and get_parent().get_node("HUD").get_node("TimerGO").is_stopped():
#		print(get_parent().get_node("HUD")._game_over())
		get_parent().get_node("HUD")._game_over()
		print(get_parent().get_node("HUD").get_node("TimerGO").is_stopped())
#	Game Over por caida DIRECTO:
